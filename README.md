...
**Samwoo-CM-Bridge**는 건설사업관리(CM) 현장에서 시공사가 제출하는 복잡한 시공계획서와 구조/수치 계산서를 검토할 때, **[국가법령(law.go.kr)] - [KCSC 건설기준(kcsc.re.kr)] - [발주처 요구조건/계산서]**의 3자 데이터를 **외부 유출 없이 로컬에서 교차 검증**하고, Python 연산을 통해 **Zero-Hallucination(수치 날조 원천 차단)** 검산을 수행한 뒤 **삼우씨엠 표준 감리의견서(.docx, .md)**를 원클릭으로 자동 생성하는 FastMCP 서버 엔진입니다.
...
1. **실시간 국가법령 및 KCSC 건설기준 연동 (Module 1)**
   ...
   - 오프라인 심사 및 시연 환경을 고려한 **지능형 Mock/Cache Fallback** 탑재.
...
2. **로컬 격리 샌드박스 & HWPX 다차원 문서 파서 (Module 2)**
   ...
   - Excel(`.xlsx`), Word(`.docx`), PowerPoint(`.pptx`), PDF(`.pdf`), TXT 완벽 지원.

3. **5대 공종 다분야 수치 검산 엔진 (Module 3)**
   - **토목/구조**: 가설 흙막이 버팀보(Strut) 좌굴, 지반 앵커, 히빙/보일링, 전도/활동 안전율.
   - **기계/설비**: 펌프 양정 여유율, 배관 마찰손실, 필요 환기량.
   - **소방**: 옥내소화전/스프링클러 유효수량 및 토출량($K\sqrt{10P}$), 제연풍량.
   - **전기/통신**: 3상 선로 전압강하율, 변압기 부하율 및 수용률, 케이블 허용전류.
   - **건축**: 외벽 열관류율($U$-value), 단열재 두께 기준, 직통계단 보행거리.
   - Python AST 기반 안전 커스텀 수식 검산 지원.

4. **삼우씨엠 표준 감리/CM 검토의견서 자동 생성**
   - 4단 3자 교차 대조표, 분야별 검산 상세표, 승인/반려 종합의견을 갖춘 고품질 `.docx` 및 `.md` 자동 조립.
...
│   ├── server.py                      # FastMCP 서버 및 7대 도구 정의

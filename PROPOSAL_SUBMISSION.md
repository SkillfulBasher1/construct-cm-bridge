...
#### ▶ 향후 핵심 과제 및 단점 보완: 사내 보안 프로그램(DRM) 암호화 문서 지원 요청
- **현재 겪고 있는 기술적 한계:** 삼우씨엠 사내 보안 프로그램(문서보안 DRM)으로 암호화가 적용된 파일(HWPX, XLSX, DOCX, PDF)의 경우, 로컬 오픈소스 파서가 복호화 키 없이 직접 읽을 수 없어 현재는 감리원이 수동으로 복호화한 후 검토를 수행해야 하는 현실적 번거로움이 있습니다.
- **본사 차원의 지원 요청 건의:** 사내 보안 정책을 철저히 준수하면서도 현장 감리원이 편리하게 활용할 수 있도록, 본사 정보보안팀과 협의하여 승인된 로컬 프로세스 내에서 암호화 문서를 안전하게 메모리 상에서 파싱할 수 있는 **'사내 공식 DRM 복호화 API 연동'** 또는 **'보안 인증 모듈 지원'**을 강력히 건의드립니다. 이 연동이 이루어지면 암호화 도서까지 원클릭으로 전수 검토하는 완전무결한 시스템으로 도약할 수 있습니다.

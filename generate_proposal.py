...
output_download_path = r"C:\Users\tbvja\Downloads\[양식2] 외부 AI 활용 아이디어 제안서_Construct-CM-Bridge.docx"
output_project_path = r"c:\Users\tbvja\Projects\samwoo-cm-bridge\[제안서] Construct-CM-Bridge_외부AI활용아이디어.docx"
...
add_heading_paragraph(c_after, "정성적 기대효과 및 건설 엔지니어링 업계 독보적 기술 리더십 확보", level=2)
add_styled_paragraph(c_after, "완전 무결한 로컬 데이터 보안: 민감한 설계 도서가 외부 클라우드로 단 한 글자도 나가지 않아, 기업 보안 감사에 100% 부합하며 모든 건설·감리 현장 즉시 배포 가능.", bullet=True)
...
add_styled_paragraph(c_after, "국내 공문서 표준 고품질 산출물 자동화: 정밀 출처 좌표(Line/Cell)가 포함된 감리의견서, 시정지시서(NCR), 일일업무일보, 준공완료보고서가 표준 공문서 서식으로 즉시 출력되어 전 현장 감리 품질 상향 평준화.", bullet=True)
...
add_heading_paragraph(c_tools, "Construct-CM-Bridge 구축에 투입된 AI 플랫폼 및 기술 스택", level=1)
...
add_styled_paragraph(c_tools, "python-docx: 표준 감리의견서, 시정지시서(NCR), 일일업무일보, 준공보고서 자동 조립", bullet=True)
...
r = p_att_head.add_run("■ [첨부자료] Construct-CM-Bridge 아키텍처 및 실제 동작 검증 요약")
format_run(r, FONT_NAME, Pt(11), bold=True, color=NAVY)

att_items = [
    ("첨부 1: 전체 시스템 아키텍처 파이프라인 (Local Sandbox & FastMCP Pipeline)",
     "• 구조: [사용자 자연어 질의] ➔ [Claude Desktop / FastMCP Client] ➔ [로컬 secure_local_data 격리 파서 (HWPX/XLSX/DOCX/PDF)] ➔ [국가법령/KCSC OpenAPI 실시간 조회] ➔ [Python AST 안전율/응력 직접 검산] ➔ [표준 감리의견서 Word/MD 자동 출력]\n• 핵심 원칙: Zero-Cloud Leak(외부 유출 제로) & Zero-Hallucination(수치 연산 무오류) & 1인 1AI 맞춤 비서"),
